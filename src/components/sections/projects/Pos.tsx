      ...
      <div className="flex h-1/2 my-10 relative max-md:hover:scale-150">
        <div className="flex gap-3 items-center max-md:flex-col">
          ...
          {showimg.map((id, index) => (
            currentimg!=1?
            index==1? <img src={`./src/img/temp/POS/${id}.JPG`} alt="" className={`duration-300 scale-100 md:scale-150 z-10 w-[250px] h-[250px]`} key={id} onClick={()=>{window.open('./src/img/temp/POS/'+id+'.JPG')}}/>: <img src={`./src/img/temp/POS/${id}.JPG`} alt="" className={`duration-300 scale-100 w-[250px] h-[250px] max-md:hidden`} key={id}/>
              :
              index==0? <img src={`./src/img/temp/POS/${id}.JPG`} alt="" className={`duration-300 scale-100 md:scale-150 z-10 w-[250px] h-[250px]`} key={id} onClick={()=>{window.open('./src/img/temp/POS/'+id+'.JPG')}}/>   : <img src={`./src/img/temp/POS/${id}.JPG`} alt="" className={`duration-300 scale-100 w-[250px] h-[250px] max-md:hidden`} key={id}/>
            ))}
