    ...
    return (
      <div className="flex flex-col w-full h-full items-center gap-5">
        <div className="flex h-1/2 my-10 relative max-md:hover:scale-150">
          <div className="flex gap-3 items-center max-md:flex-col">
            ...
            {showimg.map((id, index) => (
              currentimg!=1?
              index==1? <a href={`./src/img/temp/face/${id}.JPG`} target="_blank" className="z-10"><img src={`./src/img/temp/face/${id}.JPG`} alt="" className={`duration-300 scale-100 md:scale-150 z-10 w-[250px] h-[250px]`} key={id}/></a> : <img src={`./src/img/temp/face/${id}.JPG`} alt="" className={`duration-300 scale-100 w-[250px] h-[250px] max-md:hidden`} key={id}/>
              :
              index==0? <a href={`./src/img/temp/face/${id}.JPG`} target="_blank" className="z-10"><img src={`./src/img/temp/face/${id}.JPG`} alt="" className={`duration-300 scale-100 md:scale-150 z-10 w-[250px] h-[250px]`} key={id}/></a> : <img src={`./src/img/temp/face/${id}.JPG`} alt="" className={`duration-300 scale-100 w-[250px] h-[250px] max-md:hidden`} key={id}/>
            ))}
        ...
        <div className="px-3 text-center">
                                          ...
                                          <p className="text-xs">Face Identification Application </p>
                                          <h5>Programming Language</h5>
                                          <p className="text-xs">PHP 8</p>
                                          <h5>Framework & Tools</h5>
                                          <p className="text-xs">Laravel 9, JQuery, JQuery, Javascript, Bootstrap, MySQL</p>
                                      </div>
      </div>
    )
}
