...
export const Navbar = () => {
  ...
  const {page, dark, setdark} = useContext(Pages)
  const {setport} = useContext(Portopage)
  return (
    <div className={`w-full flex justify-between mb-5 relative ${dark? ' text-white ':''}`}>
      <div className="flex flex-wrap items-center justify-between w-full">
        <div className="flex items-center gap-2 w-full relative">
        <a href=''><img src='.\src\img\logo.png' alt="logo" className={`h-full max-h-[75px] ${dark?' invert ':''}`} /></a>
          <div>
            <a href=''><h2>BANG ONE</h2></a>
            <a href=''><h6 className=" max-md:hidden">My Porofolio Website</h6></a>
          </div>
          <button className={`font-bold rounded-lg p-1 duration-500 ${dark?' bg-white text-black ':' bg-black text-white '}`} onClick={()=>{setdark(!dark)}}>{`${dark?'Light':'Dark'}`}</button>  
          ...
          <div className="flex ms-auto items-center me-10 gap-5 border-b-2 border-gray-400 rounded-xl max-md:hidden">
            ...
            <div className="flex text-gray-400/70 gap-0 items-end animate-ping w-[75px] h-[75px]">
              <img src="./src/img/arrow.png" alt="Arrow Here" className={`mt-2 px-3 ${dark?' invert ':''} `}></img>
            </div>
